...
struct STTAudioContractTests {
  static func main() throws {
    ...
    let rawFloatOne: [UInt8] = [0, 0, 128, 63]
    try STTAudioContract.validate(byteCount: 4, prefix: rawFloatOne)
    try expect(
      STTAudioContract.detectEncodedFormat(prefix: [82, 73]) == nil,
      "a prefix shorter than any magic is not misdetected"
    )

    let rawSamplesResemblingMp3FrameSync: [UInt8] = [0xFF, 0xFB, 0x90, 0x00]
    try STTAudioContract.validate(byteCount: 8, prefix: rawSamplesResemblingMp3FrameSync)
